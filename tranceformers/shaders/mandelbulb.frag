...
float map(in vec3 pos, out vec3 orbit_trap)
{
    ...
    for (int i = 0; i < 300; ++i) {        
        ...
        // to cartesian
        z = zr * vec3(cos(theta)*cos(phi)*(1-ubo.power_to)+sin(theta)*cos(phi)*ubo.power_to, sin(phi)*sin(theta), cos(theta));        
        z += c;
...
void main()
{
    ...
    vec3 cam_pos = vec3(2.5 * cos(0.1 * freq), 0.0, 2.5*sin(0.1 * freq));
    const vec3 cam_target = vec3(0);
    float transp = 1.0;
    const float fov = 120.0 * 3.141592 / 180.0;
    float h = tan(fov/2.0) * length(cam_target - cam_pos);
    ...
    vec2 p = vec2((1.0 * fragCoord.x - iResolution.x + iResolution.x/20)/1080, (1.0 * fragCoord.y - iResolution.y + 400) / 1080);
	#else
	vec2 p = vec2((1.0 * fragCoord.x - iResolution.x/2)/1080, (1.0 * fragCoord.y - iResolution.y/2) / 1080);
    #endif
	...
	*/
    ...
    {
        transp = 0.0;
    	col = vec3(0.0, 0.0471, 0.0471); //texture(inTexture, vec2(pass_Uvs.x, pass_Uvs.y)).xyz;
	}
#if AA
    final_col += col;
    }
    ...
    out_Color = vec4(vec3(final_col), transp);
}