...
namespace nap
{
	...
	FoglioService::FoglioService(ServiceConfiguration* configuration) :
	...
	bool FoglioService::init(nap::utility::ErrorState& errorState)
	{
		...
		mInternalScene = std::make_unique<Scene>(getCore());
	...
	/*nap::SpawnedEntityInstance FoglioService::spawn(nap::Entity& entity, nap::utility::ErrorState& error)
	{
		assert(mInternalScene != nullptr);
		nap::Logger::error("FoglioService: spawning in internal scene %s", mInternalScene->mID.c_str());
		return mInternalScene->spawn(entity, error);
	}*/

	void FoglioService::destroy(SpawnedEntityInstance& entityInstance) {
		assert(mInternalScene != nullptr);
		mInternalScene->destroy(entityInstance);
	}

	void FoglioService::postUpdate(double deltaTime)
	{
		mInternalScene->updateTransforms(deltaTime);
	}
	...
	//		addedIdIfExisting++;
	//	}
	//	std::unique_ptr<Entity> videoRenderEntity = getCore().getResourceManager()->createObject<Entity>();
	...
	//	mVideoRenderSpawnedEntityInstance = mInternalScene->spawn(*videoRenderEntity, errorState);
